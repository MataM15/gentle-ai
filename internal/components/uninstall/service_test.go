...
func TestComponentOperationsOpenCodeGentleLogoRespectsXDGConfigHome(t *testing.T) {
	homeDir := t.TempDir()
	workspaceDir := t.TempDir()
	xdgDir := t.TempDir()
	...
	logoPath := filepath.Join(xdgDir, "opencode", "tui-plugins", "gentle-logo.tsx")
	if err := os.MkdirAll(filepath.Dir(logoPath), 0o755); err != nil {
		t.Fatalf("MkdirAll(logo dir) error = %v", err)
	}
	if err := os.WriteFile(logoPath, []byte("// managed logo"), 0o644); err != nil {
	...
	fallbackLogoPath := filepath.Join(homeDir, ".config", "opencode", "tui-plugins", "gentle-logo.tsx")

	ops, targets, err := svc.componentOperations(adapter, model.ComponentOpenCodeGentleLogo)
	if err != nil {
		t.Fatalf("componentOperations() error = %v", err)
	}
	if !slices.Contains(targets, logoPath) {
		t.Fatalf("targets = %#v, want XDG-scoped logo path %q", targets, logoPath)
	}
	if slices.Contains(targets, fallbackLogoPath) {
		t.Fatalf("targets = %#v, must not include HOME fallback path %q", targets, fallbackLogoPath)
	}
	for _, op := range ops {
		if _, _, err := op.apply(op.path); err != nil {
	...
	if _, err := os.Stat(logoPath); !os.IsNotExist(err) {
		t.Fatalf("XDG-scoped logo file should be removed; stat err = %v", err)
	}
}

func TestComponentOperationsSDD_OpenCodeRemovesDepDirUnderXDGConfigHome(t *testing.T) {
	homeDir := t.TempDir()
	workspaceDir := t.TempDir()
	xdgDir := t.TempDir()
	t.Setenv("HOME", homeDir)
	...
	depDir := filepath.Join(xdgDir, "opencode", "node_modules", "unique-names-generator")
	if err := os.MkdirAll(depDir, 0o755); err != nil {
		t.Fatalf("MkdirAll(depDir) error = %v", err)
	}
	if err := os.WriteFile(filepath.Join(depDir, "index.js"), []byte("module"), 0o644); err != nil {
		t.Fatalf("WriteFile(depDir file) error = %v", err)
	}
	fallbackDepDir := filepath.Join(homeDir, ".config", "opencode", "node_modules", "unique-names-generator")

	ops, targets, err := svc.componentOperations(adapter, model.ComponentSDD)
	if err != nil {
		t.Fatalf("componentOperations() error = %v", err)
	}
	if !slices.Contains(targets, depDir) {
		t.Fatalf("targets = %#v, want XDG-scoped dep dir %q", targets, depDir)
	}
	if slices.Contains(targets, fallbackDepDir) {
		t.Fatalf("targets = %#v, must not include HOME fallback dep dir %q", targets, fallbackDepDir)
	}
	for _, op := range ops {
		if _, _, err := op.apply(op.path); err != nil {
			t.Fatalf("op.apply(%q) error = %v", op.path, err)
	...
	if _, err := os.Stat(depDir); !os.IsNotExist(err) {
		t.Fatalf("XDG-scoped dep dir should be removed; stat err = %v", err)
	}
}
