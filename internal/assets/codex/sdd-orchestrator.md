...
1. Before every runtime-bearing apply, verify, or remediation launch, classify it and call `gentle-ai sdd-attempt acquire --cwd <repo> --change <change> --request-id <id> --work-unit <label> --evidence-goal <goal> --max-attempts <count> --max-changed-lines <count> --attempt-class <environment|harness|acceptance>`. Use `environment` only for readiness outside the harness; use `harness` only for harness/evidence readiness after environment passes; use `acceptance` only for product/spec behavior after both pass. Project-specific checks remain repo-owned.
2. Launch only when acquire returns `state: proceed`, and retain its opaque `token`. `blocked` or `complete` stops the launch.
3. After the external run, call `gentle-ai sdd-attempt settle --cwd <repo> --change <change> --token <token> --request-id <settle-id> ...` with a request ID distinct from the acquire operation's request ID. Settle preserves the token's class; failures charge only that class, never acceptance by default. Reuse each operation's own ID only for idempotent replay. Settle derives native binding/remediation inputs; pass `--successor-lineage` only for a distinct approved successor, otherwise the bound lineage remains its own successor.
